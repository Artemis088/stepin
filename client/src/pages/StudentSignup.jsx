...
import { useToast, Icon, Chip } from '../components/ui.jsx';
...
export default function StudentSignup() {
  ...
  const createAccount = async () => {
    if (!form.email || !form.password || !form.name) return toast.error('Fill in name, email and password');
    setBusy(true);
    try {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#eaece7', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: 'var(--surface-0)', border: '0.5px solid var(--border)', borderRadius: 16, width: '100%', maxWidth: 540, padding: 30 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 22 }}>
          {STEPS.map((s, i) => (
            <div key={s} style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 10.5, color: i <= step ? 'var(--amber-700)' : 'var(--text-muted)', marginTop: 5 }}>{s}</div>
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <>
            <h1 style={{ fontSize: 20, marginBottom: 14 }}>Basic info</h1>
            <div className="field">
              <label>Your field (MVP: one vertical)</label>
            ...
            <div className="field"><label>Short bio</label><input value={form.bio} onChange={set('bio')} placeholder="3rd-year student · exploring analytics" /></div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button className="ghost" onClick={() => setStep(0)}>Back</button>
              <button className="primary-amber" style={{ flex: 1 }} onClick={createAccount} disabled={busy}>{busy ? 'Creating…' : 'Create account'}</button>
            </div>
          </>
        )}
        ...
        {step === 2 && (
          <>
            <h1 style={{ fontSize: 20, marginBottom: 4 }}>Add your skills</h1>
            <div style={{ background: 'var(--amber-50)', color: 'var(--amber-900)', borderRadius: 10, padding: '11px 14px', fontSize: 12.5, lineHeight: 1.55, marginBottom: 16, display: 'flex', gap: 8 }}>
              ...
              <span>Unverified skills are visible but don't boost your ranking until you back them with evidence. You can always earn verification by completing a StepIn task.</span>
            </div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
              <input value={skillDraft} onChange={(e) => setSkillDraft(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addSkill()} placeholder="e.g. SQL" />
              <button className="ghost" onClick={addSkill}><Icon name="plus" size={15} /> Add</button>
            </div>
                  ...
                  <div key={i} style={{ border: `0.5px ${verified ? 'solid var(--border)' : 'dashed var(--border-strong)'}`, borderRadius: 8, padding: '10px 12px' }}>
                    <div className="spread">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        {verified ? <Chip tone="teal" icon="rosette-discount-check">Verified</Chip> : <Chip tone="outline">Unverified</Chip>}
                      </div>
                      <button className="link" onClick={() => setSkills((s) => s.filter((_, idx) => idx !== i))}><Icon name="x" size={15} color="var(--text-muted)" /></button>
                    </div>
                    <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                      <button className={`sm ${sk.evidenceType === 'certificate' ? 'primary-teal' : 'ghost'}`} onClick={() => setEvidence(i, 'certificate', 'Certificate')}>Certificate</button>
                      <button className={`sm ${sk.evidenceType === 'course' ? 'primary-teal' : 'ghost'}`} onClick={() => setEvidence(i, 'course', 'Course')}>Course link</button>
                      <button className={`sm ${sk.evidenceType === 'none' ? 'ghost' : 'ghost'}`} onClick={() => setEvidence(i, 'none')} style={sk.evidenceType === 'none' ? { borderColor: 'var(--amber)' } : {}}>Earn by task</button>
                    </div>
                  </div>
                );
              })}
              {!skills.length && <div className="empty" style={{ padding: 20 }}>No skills yet — add one above.</div>}
            </div>
            <button className="primary-amber" style={{ width: '100%', height: 42 }} onClick={finish} disabled={busy}>{busy ? 'Saving…' : 'Finish'}</button>
          </>
        )}
        ...
        {step === 3 && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <h1 style={{ fontSize: 22, margin: '14px 0 8px' }}>You're all set</h1>
            <p className="secondary" style={{ marginBottom: 22 }}>Your profile is live. Browse tasks in your field and do a screening to get started.</p>
            <button className="primary-amber" style={{ height: 42, padding: '0 24px' }} onClick={() => navigate('/student/tasks')}>Browse tasks <Icon name="arrow-right" size={16} /></button>
          </div>
        )}
      </div>
    </div>
